...
public class Graph<T> {
	...
	private List<List<T>> finalResult = new ArrayList<List<T>>();
...
//			当这条路径是一条新的路径就保存，如果已经存在就不保存
	...
	public List<List<T>> startSearch(T startNodeId,T endNodeId) {
//		countPathNumber()：矩阵中有多少个分岔的点，也就是路径多于一条的点
...
//			当这条路径是一条新的路径就保存，如果已经存在就不保存
			if(!containBranch(result,path)){
//				截取开始节点和结束节点之间的节点
				int startIndex = path.indexOf(startNodeId);
				int endIndex = path.indexOf(endNodeId);
				if(startIndex<endIndex){
					List<T> subList = path.subList(startIndex+1, endIndex);
					finalResult.add(subList);
					// 保存结果
					result.add(path);
				}
