...
public class StringUtil {
	...
	public static long getBandwidth(String value, String formatUnit){
        ...
        if(formatUnit.equalsIgnoreCase("K")||formatUnit.equalsIgnoreCase("KB") || formatUnit.equalsIgnoreCase("Kbps") || formatUnit.equalsIgnoreCase("Kpps") || formatUnit.equalsIgnoreCase("Ksess"))
        {
            factor = 1000;
        }
        if(formatUnit.equalsIgnoreCase("M")||formatUnit.equalsIgnoreCase("MB") || formatUnit.equalsIgnoreCase("Mbps") || formatUnit.equalsIgnoreCase("Mpps") || formatUnit.equalsIgnoreCase("Msess"))
        {
            factor = 1000000;
        }
        if(formatUnit.equalsIgnoreCase("G")||formatUnit.equalsIgnoreCase("GB") || formatUnit.equalsIgnoreCase("Gbps") || formatUnit.equalsIgnoreCase("Gpps") || formatUnit.equalsIgnoreCase("Gsess"))
        {
            factor = 1000000000;
        }
        if(formatUnit.equalsIgnoreCase("T")||formatUnit.equalsIgnoreCase("TB") || formatUnit.equalsIgnoreCase("Tbps") || formatUnit.equalsIgnoreCase("Tpps") || formatUnit.equalsIgnoreCase("Tsess"))
        {
            factor = 1000000000;
        }
	...
	public static Double getBandSpeed(String value, String formatUnit){
		if(isNullString(value)||isNullString(formatUnit)){
			return 0D;
		}
        Double factor = 0d;
        ...
        factor = (Double.valueOf(value) * factor) ;
        return factor;
    }
	...
	public static Map<String,Object> getParamTime(String stime,String etime){
		Map<String,Object> maptime=new HashMap<String,Object>();
		long endTime=0L;
		long startTime=0L;
		String stTime="";
		String enTime="";
		if(!isNullString(stime)&&!StringUtil.isNullString(etime)){
			stTime=stime;
			enTime=etime;
			startTime=DateUtil.getLongTime(stTime);
			endTime=DateUtil.getLongTime(enTime);
		}else{
			endTime=System.currentTimeMillis();
			startTime =endTime-3600*1000*24L;
			stTime = DateUtil.getTimeShow(new Timestamp(startTime));
			enTime = DateUtil.getTimeShow(new Timestamp(endTime));
			startTime=DateUtil.getLongTime(stTime);
			endTime=DateUtil.getLongTime(enTime);
		}
		maptime.put("endTime", endTime);
		maptime.put("startTime", startTime);
		maptime.put("stTime", stTime);
		maptime.put("enTime", enTime);
		return maptime;
	}
	

	//获取时间跨度
		public static String getTimeFrameStr(String timeframe){
			String timestr="最近24小时";
			if(isNullString(timestr))
				return timestr;
			if(timeframe.equals("Hourly"))
				timestr="最近1小时";
			if(timeframe.equals("SixHours"))
				timestr="最近6小时";
		...
		public static long getBandwidth(String value){
			if(StringUtil.isNullString(value)){
				return 0;
			}
			String lowUnit=value.toLowerCase();
			Pattern m= Pattern.compile("m");
			Matcher mmat = m.matcher(lowUnit);
			
			Pattern k= Pattern.compile("k");
			Matcher kmat = k.matcher(lowUnit);
			
			Pattern g= Pattern.compile("g");
			Matcher gmat = g.matcher(lowUnit);
	        ...
	        if(!StringUtil.isNullString(d)){
	        	num=Integer.valueOf(d);
	        }
	        
	        Long factor = num*1000000L;
		...
		public static boolean isNumeric(Object str){
			...
			if(isNullString(ss))
				return false;
		    return ss.matches("^[+]?(([0-9]+)([.]([0-9]+))?|([.]([0-9]+))?)$");    
		 } 
		
		
		@SuppressWarnings("finally")
		public final static long getCRC32(String input) 
		   {
		       long output = 0;
		       try {
		           ...
		           ex.printStackTrace();
		           throw new Exception("Calculate CRC-32 RDN value[" + input + "] failed, reason: " + ex.getMessage(), ex);
		       } finally {
		           return output;
		       }
		   } 
