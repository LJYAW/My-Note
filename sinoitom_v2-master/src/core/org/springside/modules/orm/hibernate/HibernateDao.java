 ...
 * <p>
 * $Id: HibernateDao.java,v 1.4 2013/01/04 09:16:07 sujp Exp $
 */
...
public class HibernateDao<T, ID extends Serializable> extends SimpleHibernateDao<T, ID> {
	...
	public Page<T> findAjaxPage(final PageRequest pageRequest, String hql, final Map<String, ?> values) {
		Validate.notNull(pageRequest, "page不能为空");

		Page<T> page = new Page<T>(pageRequest);

		if (pageRequest.isCountTotal()) {//是否计算总记录数
			long totalCount = countHqlResult(hql, values);//计算总记录数
			page.setTotalItems(totalCount);
		}

		if (pageRequest.isOrderBySetted()) {//是否设置了排序字段
			hql = setOrderDirToHql(hql, pageRequest);//拼排序的hql语句，也就是加上order by
		}
		Query q = createQuery(hql, values);
		setPageParameterToQuery(q, pageRequest);//拼分页查询的hql语句，从哪开始到哪里结束
	 ...
	 * 在HQL的后面添加分页参数定义的orderBy, 辅助函数.
	 */
	protected String setOrderDirToHql(final String hql, final PageRequest pageRequest) {
		StringBuilder builder = new StringBuilder(hql);
		builder.append(" order by ");
		builder.append(pageRequest.getOrderBy()).append(" ").append(pageRequest.getOrderDir());
		return builder.toString();
	}


	private String prepareCountHql(String orgHql) {
		return "select count(*) " + removeSelect(removeOrders(orgHql));
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<T> find(List<PropertyFilter> filters, String orderBy, String orderDir) {
		Criterion[] criterions = buildCriterionByPropertyFilter(filters);
		Criteria c = createCriteria(criterions);
		String[] arrOrderBy = orderBy.split(",");
		String[] arrOrderDir = orderDir.split(",");
		for (int i = 0; i < arrOrderDir.length; i++) {
			if (Sort.ASC.equals(arrOrderDir[i])) {
				c.addOrder(Order.asc(arrOrderBy[i]));
			} else {
				c.addOrder(Order.desc(arrOrderBy[i]));
			}
		}
		return c.list();
	}
	...
	public Criterion buildCriterion(final String propertyName, final Object propertyValue, final MatchType matchType) {
		...
		switch (matchType) {
			...
			case GT:
				...
				break;
			case IN:
				criterion = Restrictions.in(propertyName, propertyValue.toString().split(SPLIT_FLAG));
				break;
			case NOTIN:
				criterion = Restrictions.not(Restrictions.in(propertyName, propertyValue.toString().split(SPLIT_FLAG)));
				break;
			case ISNULL:
