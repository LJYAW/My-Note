...
function pageInit(){
		jQuery("#list").jqGrid( {
			 url:'${ctx}/ack/devCheckItems/search.do?id='+'${id}',
				//  datatype:从服务器端返回的数据类型，默认xml。可选类型：xml，local(客户端数据（数组）)，json，jsonnp，script，xmlstring，jsonstring，clientside
				 datatype : "json", 
     
         		
	 colNames : [ '设备类型', '设备IP', '指标组', '指标英文名称', '指标显示名称', '数据类型', '长度', '小数位数', '度量单位','parseRuldId', '巡检命令','审核状态','审核状态'],
	 colModel : [ 
	        	 {name : 'resTypeName',index : 'resTypeName',width : 60},
	        	 {name : 'devIp',index : 'devIp',width :120}, 
		...
		function delRow(id)
		{
			var url = '${ctx}/ack/devCheckItems/delete.do?';
			$.ajax({
				url: url,
				type:"POST",
				dataType: 'json',
				data:{"devChkItemIds":id},
	     ...
	     {
	     	var url = '${ctx}/ack/devCheckItems/saveAudit.do?devChkItemIds='+ids;
	     	$.ajax({
	     		url: url,
	     		dataType: 'json',
	     		success: function(data) {
		...
		function onView(){
			   ...
			   layer_show("巡检指标详情",'${ctx}/ack/devCheckItems/view.do?id=' +rowid,750,500);
			}
