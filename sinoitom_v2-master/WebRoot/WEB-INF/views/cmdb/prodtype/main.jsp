...
function pageInit(){
	 ...
	 colNames : [ 'id','厂商标识', '厂商名称', '产品分类编号', '产品分类名称', '类型编号', '类型名称', '类型描述','审核状态','审核状态' ],
	 colModel : [ 
	        	 {name : 'id',index : 'id',width : 30,hidden:true},
	        	 {name : 'vendorID',index : 'vendorID',width : 30},
	        	 {name : 'vendorName',index : 'vendorName',width : 30}, 
	        	 {name : 'prodClassCode',index : 'prodClassCode',width : 30},
	        	 {name : 'prodClassName',index : 'prodClassName',width: 60},
	        	 {name : 'typeCode',index : 'typeCode',width : 60},
	        	 {name : 'dispName',index : 'dispName',width : 60},
		...
		function onadd(){
			 ...
			 layer_show("添加产品类型",'${ctx}/cmdb/prodType/add.do?id=' + '${id}',800,600)
			}  
		function onEdit()
		{
		 ...
		 if (rowData.status==1) { 
			...
			editRow(rowData.vendorID,rowData.prodClassCode,rowData.id);
			
		}
		function editRow(id,code,ID)
		{
			layer_show("修改机构信息",'${ctx}/cmdb/prodType/edit.do?vendorId=' + id + '&&code=' + code + '&&id=' + ID,800,600)
		}
		...
		function delRow(id)
		{
			var url = '${ctx}/cmdb/prodType/delete.do';
			$.ajax({
				url: url,
				type:"POST",
				dataType: 'json',
	     ...
	     {
	     	var url = '${ctx}/cmdb/prodType/saveAudit.do?prodTypeIds='+ids;
	     	$.ajax({
	     		url: url,
	     		dataType: 'json',
	     		success: function(data) {
		...
		function onImport() {	
            ...
            layer_show("导入产品类型表",'${ctx}/cmdb/prodType/importGo.do',720, 220)
