...
function pageInit(){
	 ...
	 colNames : [ '监测对象', '监测对象', '指标维度', '指标组', '阀值分类', '缺省单位', '单位名称', '状态', '描述'],
	 colModel : [ 
	        	 {name : 'id',index : 'id' ,width : 30,hidden:true},
	        	 {name : 'indObjName',index : 'indObjName',width : 80}, 
	        	 {name : 'indDimName',index : 'indDimName',width : 100},
	        	 {name : 'indGroupName',index : 'indGroupName',width: 100},
	        	 {name : 'threshTypeName',index : 'threshTypeName',width : 100},
	        	 {name : 'defaultUnit',index : 'defaultUnit',width : 100},
	        	 {name : 'unitName',index : 'unitName',width : 100},
	        	 {name : 'status',index : 'status',width : 50,
	        		 formatter:function(cellvalue, options, row){
	        			 var html = '';
	        			 if(row.status==1){
	        				 html += '<span id='+row.id+' '+row.id+'='+row.status+' style="color:blue;">已审核</span>';
	                     }else{
	                        html += '<span id='+row.id+' '+row.id+'='+row.status+' style="color:red;">未审核</span>';
	                     }
	        			 return html;
	        		 }	 
	        	 },
		...
		function onadd(){
			 ...
			 layer_show("添加",'${ctx}/cmdb/thresholdType/add.do',800,400)
			}  
			function onEdit()
			{
		 ...
		 if (rowid==""||rowid==null) { 
		 	layer.msg('请选择一条阀值分类',{icon: 7,time:1500});
		 	return; 
		 }
			editRow(rowid);
		...
		function editRow(id)
		{
			layer_show("修改信息",'${ctx}/cmdb/thresholdType/edit.do?id='+id,800,450)
		}
		...
		function delRow(id)
		{
			var url = '${ctx}/cmdb/thresholdType/delete.do?id='+id;
			$.ajax({
				url: url,
				dataType: 'json',
