...
function pageInit(){
	 ...
	 colNames : [ '产品厂商ID', '产品厂商','产品分类', '产品类型', '产品型号', '指标名称', 'Snmp对象名称', 'SnmpOID', '采集方式', 'OID标志', '度量单位', '审核状态', '审核状态'],
	 colModel : [ 
	        	 ...
	        	 {name : 'indItemName',index : 'indItemName',width :120},
	        	 {name : 'snmpObjName',index : 'snmpObjName',width : 120},
	        	 {name : 'snmpOID',index : 'snmpOID',width : 120},
	        	 {name : 'getMethod',index : 'getMethod',width : 40},
		...
		function onadd(){
			 ...
			 layer_show("添加指标采集OID信息",'${ctx}/cmdb/prodIndItem/add.do?pid='+'${pid}',800,450)
			}  
			function onEdit()
			{
		...
		function editRow(id)
		{
			layer_show("修改信息",'${ctx}/cmdb/prodIndItem/edit.do?id='+id,800,450)
		}
		...
		function delRow(id)
		{
			var url = '${ctx}/cmdb/prodIndItem/delete.do?';
			$.ajax({
				url: url,
				type:"POST",
				dataType: 'json',
		...
		//审核
	     ...
	     {
	     	var url = '${ctx}/cmdb/prodIndItem/saveAudit.do?prodIndItemIds='+ids;
	     	$.ajax({
	     		url: url,
	     		dataType: 'json',
	     		success: function(data) {
