...
<div id="btn" style="margin-top:2px;">
	...
	<shiro:hasPermission name="IndGroup_Import">
...
function pageInit(){
	 ...
	 colNames : [ '分类编码', '监测对象', '指标组英文名称', '指标组名称', '指标组描述','审核状态','审核状态' ],
	 colModel : [ 
	        	 {name : 'indClassCode',index : 'indClassCode',width : 60},
	        	 {name : 'indClassName',index : 'indClassName',width : 80}, 
	        	 {name : 'groupEnName',index : 'groupEnName',width : 200},
		...
		function onadd(){
			 ...
			 layer_show("添加指标组信息",'${ctx}/cmdb/indicator/add.do?classCode='+classCode,800,400)
			}  
			function onEdit()
			{
		...
		function editRow(id)
		{
			layer_show("修改指标组信息",'${ctx}/cmdb/indicator/edit.do?id='+id,800,450)
		}
		...
		function delRow(id)
		{
			var url = '${ctx}/cmdb/indicator/delete.do?';
			$.ajax({
				url: url,
				type:"POST",
				dataType: 'json',
		...
		//审核
	     ...
	     function audit(ids)
	     {
	    	 layer.confirm('确认要审核吗？', function (yes) {
	 			auditRow(ids);
	     ...
	     {
	     	var url = '${ctx}/cmdb/indicator/saveAudit.do?groupIds='+ids;
	     	$.ajax({
	     		url: url,
	     		dataType: 'json',
	     		success: function(data) {
		...
		function onImport() {	
            ...
            layer_show("导入指标组表",'${ctx}/cmdb/indicator/importGo.do',720, 220)
