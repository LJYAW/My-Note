...
function pageInit(){
	 ...
	 colNames : [ '厂商标识', '厂商名称', '操作系统分类', '操作系统类型', '操作系统描述','审核状态','审核状态' ],
	 colModel : [ 
	        	 {name : 'vendorID',index : 'vendorID',width : 30},
	        	 {name : 'vendorName',index : 'vendorName',width : 30}, 
	        	 {name : 'osClassName',index : 'osClassName',width : 30},
	        	 {name : 'osType',index : 'osType',width: 60},
		...
		function onadd(){
			 ...
			 layer_show("添加操作系统类型",'${ctx}/cmdb/os/add.do?vendorId=' + vendorId,800,350)
			}  
			function onEdit()
			{
		...
		function editRow(id)
		{
			layer_show("修改操作系统类型",'${ctx}/cmdb/os/edit.do?id='+id,800,350)
		}
		...
		<%--    			doData(data);--%>
	     ...
	     {
	     	var url = '${ctx}/cmdb/os/saveAudit.do?OSIds='+ids;
	     	$.ajax({
	     		url: url,
	     		dataType: 'json',
	     		success: function(data) {
