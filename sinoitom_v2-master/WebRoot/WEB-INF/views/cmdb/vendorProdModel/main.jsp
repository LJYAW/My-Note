...
function pageInit(){
	 ...
	 colNames : [ '厂商名称', '产品分类', '产品类型', '产品系列', '产品型号', '型号OID','型号描述','状态','状态'],
	 colModel : [ 
	        	 {name : 'vendorName',index : 'vendorName',width : 30},
	        	 {name : 'prodClassName',index : 'prodClassName',width : 30}, 
	        	 {name : 'prodTypeName',index : 'prodTypeName',width : 30},
	        	 {name : 'prodSeries',index : 'prodSeries',width: 40,
	        		 formatter:function(cellvalue, options, row){
	        			 var html = '';
	        			 if(row.prodSeries== -1 ||row.prodSeries == null){
	        				 html = '未定义';
	        			 }else{
	        				 html += row.prodSeries;
	        			 }
	        	 ...
	        	 {name : 'prodModelName',index : 'prodModelName',width : 60},
	        	 {name : 'prodModelOid',index : 'prodModelOid',width : 60},
		...
		function onadd(){
			 ...
			 layer_show("添加产品型号",'${ctx}/cmdb/vendorProdModel/add.do?id=' + '${id}',800,400)
			}  
			function onEdit()
			{
		...
		function editRow(id)
		{
			layer_show("修改机构信息",'${ctx}/cmdb/vendorProdModel/edit.do?id='+id,800,450)
		}
		...
		function delRow(id)
		{
			var url = '${ctx}/cmdb/vendorProdModel/delete.do?';
			$.ajax({
				url: url,
				type:"POST",
				dataType: 'json',
	     ...
	     {
	     	var url = '${ctx}/cmdb/vendorProdModel/saveAudit.do?prodModelIds='+ids;
	     	$.ajax({
	     		url: url,
	     		dataType: 'json',
	     		success: function(data) {
		...
		function onImport() {	
            ...
            layer_show("导入产品分类表",'${ctx}/cmdb/vendorProdModel/importGo.do',720, 220)
