...
<script type="text/javascript">
 ...
 function refreshVendor() {
              ...
              function (result) {
                  if (result.vendors != null) {
                      ...
                      for (var i = 0; i < result.vendors.length; i++) {
                          ...
                          if(vendor.vendorID=='${prodModel.vendorID}'||vendor.vendorID=='${vendorID}'){
 	...
 	function refreshClassCode() {
              ...
              function (result) {
                  if (result.prodType != null) {
                      ...
                      for (var i = 0; i < result.prodType.length; i++) {
                          ...
                          if(prodType.typeCode=='${prodModel.prodTypeCode}'){
              ...
              function (result) {
                  if (result.seriesList != null) {
                      ...
                      if ("add" == '${action}'){
	                      ...
	                      $("#prodSeries").append("<option  value=\"" + "-1" + "\">" + "未定义" + "</option>");
                      }else{
                      	  for (var i = 0; i < result.seriesList.length; i++) {
	                          var series = result.seriesList[i];
	                          var seriesEqOne = true;//标志位用来判断 产品系列 是否为“其他”
	                          if(series.prodSeries=='${prodModel.prodSeries}'){
	                            $("#prodSeries").append("<option  value=\"" + series.prodSeries + "\" selected>" + series.prodSeries + "</option>");
	                          }else{
	                            $("#prodSeries").append("<option  value=\"" + series.prodSeries + "\">" + series.prodSeries + "</option>");
	                      ...
	                      if(seriesEqOne){
	                      	...
	                      	$("#prodSeries").append("<option  value=\"" + "-1" + "\"  selected>" + "未定义" + "</option>");
	                      }
  	...
  	//验证同一厂商是否有重复的产品型号名称
  	function checkProdModel(){
  	var vendorId=$("#vendorID").find("option:selected").val();
  	var prodModel=$("#prodModel").val();
  	
  	 $.getJSON(
              "${ctx}/cmdb/vendorProdModel/checkProdModel.do?vendorId="+vendorId+"&prodModel="+prodModel,
              function (result) {
                  if (result.models != null&&result.models !="") {
                       window.parent.$.ligerDialog.error('该系列产品型号已存在,请重新填写！'); 
                       $("#prodModel").val("");
                     }   
              });
...
<body> 
	<form:form id="formSave" modelAttribute="prodModel" action="${ctx}/cmdb/vendorProdModel/save.do" method="post">
	<input type=hidden name="action" value="${action}"/>
	<input type="hidden" name="id" value="${id}"/>
	<input type="hidden" name="status" value="${prodModel.status}"/>
	<input type="hidden" name="vendorName" id="vendorName" value="${prodModel.vendorName}"/>
		<table align="center" class="tb_edit">
            ...
            <tr>
                ...
                <td>
                 <select id="vendorID" name="vendorID" style="width:202px" onchange="refreshClassCode();selectVendorName()">
             ...
             <tr>
                ...
                <td>
                 ...
                 <input type="hidden" name="prodTypeName" id="typeName" value="${prodModel.prodTypeName}"/>
                 <input type="hidden" name="prodTypeCode" id="typeCode" value="${prodModel.prodTypeCode}"/>
                </td>
               <td></td>
            ...
            <tr>
                ...
                <td><input id="prodModelName" name="prodModelName" value="${prodModel.prodModelName}" type="text" ltype="text" class="validate[required[产品型号不能为空]] ip1" onchange=" checkProdModel()"/></td>
                <td class="required"></td>
                
                <td align="right">型号OID：</td>
                <td><input id="prodModelOid" name="prodModelOid" value="${prodModel.prodModelOid}" type="text" ltype="text" class="validate[required[型号OID不能为空]] ip1" /></td>
                <td class="required"></td>
            </tr>
            
            <tr>
                <td align="right">型号描述：</td>
                <td colspan="4"><input id="description" name="description" value="${prodModel.description}" type="text" ltype="text" class="ip2" /></td>
                <td></td>
            </tr>
            <tr>
            	<td>&nbsp;&nbsp;</td>
