...
function pageInit(){
	 ...
	 colNames : ['厂商名称', '产品分类名称', '产品类型', '产品系列', '系列描述','审核状态','审核状态'],
	 colModel : [ 
	        	 {name : 'vendorID',index : 'vendorID',width : 30},
	        	 {name : 'prodClassCode',index : 'prodClassCode',width : 30}, 
	        	 {name : 'prodTypeID',index : 'prodTypeID',width : 30},
	        	 {name : 'prodSeries',index : 'prodSeries',width: 60},
		...
		function onadd(){
			 ...
			 layer_show("添加产品系列",'${ctx}/cmdb/vendorProdSeries/add.do?id=' + '${id}',800,400)
			}  
			function onEdit()
			{
		...
		function editRow(id)
		{
			layer_show("修改产品系列",'${ctx}/cmdb/vendorProdSeries/edit.do?id='+id,800,450)
		}
		...
		function delRow(id)
		{
			var url = '${ctx}/cmdb/vendorProdSeries/delete.do';
			$.ajax({
				url: url,
				type:"POST",
				dataType: 'json',
		...
		<%--    			doData(data);--%>
	     ...
	     {
	     	var url = '${ctx}/cmdb/vendorProdSeries/saveAudit.do?prodSeriesIds='+ids;
	     	$.ajax({
	     		url: url,
	     		dataType: 'json',
	     		success: function(data) {
		...
		function onImport() {	
            ...
            layer_show("导入产品系列表",'${ctx}/cmdb/vendorProdSeries/importGo.do',720, 220)
