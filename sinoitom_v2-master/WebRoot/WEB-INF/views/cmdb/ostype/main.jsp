...
function pageInit(){
	 ...
	 colNames : [ '厂商标识', '厂商名称', '操作系统分类编码', '操作系统分类', '描述', '审核状态','审核状态'],
	 colModel : [ 
	        	 {name : 'vendorID',index : 'vendorID',width : 30},
	        	 {name : 'vendorName',index : 'vendorName',width : 30}, 
	        	 {name : 'osClassCode',index : 'osClassCode',width : 60},
	        	 {name : 'osClassName',index : 'osClassName',width: 60},
		...
		function onadd(){
			 ...
			 layer_show("添加操作系统分类",'${ctx}/cmdb/ostype/add.do?vendorId=' + vendorId,800,350)
			}  
			function onEdit()
			{
		...
		function editRow(id)
		{
			layer_show("修改操作系统分类",'${ctx}/cmdb/ostype/edit.do?id='+id,800,450);
		}
		...
		function delRow(id)
		{
			var url = '${ctx}/cmdb/ostype/delete.do';
			$.ajax({
				url: url,
				type:"POST",
				dataType: 'json',
		...
		<%--    			doData(data);--%>
	     ...
	     {
	     	var url = '${ctx}/cmdb/ostype/saveAudit.do?osTypeIds='+ids;
	     	$.ajax({
	     		url: url,
	     		dataType: 'json',
	     		success: function(data) {
