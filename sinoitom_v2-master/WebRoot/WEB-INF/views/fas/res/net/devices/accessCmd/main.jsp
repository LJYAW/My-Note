...
<script type="text/javascript">
    ...
    $(function () {
        ...
        jQuery("#list").jqGrid({
            data: ${jsonListData},
            datatype: "local",
            colNames: ['厂商','操作系统类型', '操作系统名称', '操作系统版本','用户提示符','密码提示符','命令提示符','特权命令','特权密码提示符','特权模式提示符','操作'],
            colModel: [
            	{name: 'vendorName', index: 'vendorName', width: 80},
                {name: 'osType', index: 'osType', width: 80},
                {name: 'osName', index: 'osName', width: 90},
                {name: 'osVersion', index: 'osVersion', width: 100},
                {name: 'userAcsPrompt', index: 'userAcsPrompt', width: 100},
                {name: 'passwdPrompt', index: 'passwdPrompt', width: 100},
                {name: 'userModePrompt', index: 'userModePrompt', width: 100},
                {name: 'privModeCmd', index: 'privModeCmd', width: 100},
                {name: 'privPasswdPrompt', index: 'privPasswdPrompt', width: 100},
                {name: 'privModePrompt', index: 'privModePrompt', width: 100},
...
//          multiboxonly: true,
        ...
        {
        	var url = '${ctx}/devices/accessCmd/getData.do';
        	$.ajax({
        		url: url,
        		dataType: 'json',
        ...
        function onAdd()
        {
        	var url = '${ctx}/devices/accessCmd/add.do';
     		layer_show("添加设备访问信息", url, 800, 540);
        }
        ...
        function onEdit()
        {
            ...
            if (!selectRowData) { 
            	window.parent.$.ligerDialog.warn('请选择要修改的数据！'); 
            	return; 
            }
        ...
        function editRow(id)
        {
        	var url = '${ctx}/devices/accessCmd/edit.do?id='+id;
     		layer_show("修改设备访问信息", url, 800, 540); 
        }
        ...
        function delRow(id)
        {
        	var url = '${ctx}/devices/accessCmd/delete.do?id='+id;
        	$.ajax({
        		url: url,
        		dataType: 'json',
        		success: function(data) {
...
<div id="btn" style="margin-top:2px;">
   ...
   <shiro:hasPermission name="DevPromptInfo_Add">
    ...
    <shiro:hasPermission name="DevPromptInfo_Motify">
    ...
    <shiro:hasPermission name="DevPromptInfo_Delete">
    <button class="btn btn-white btn-warning btn-bold" onclick="onDelete()">
        <i class="ace-icon fa fa-trash-o bigger-120"></i>
        	删除
    </button>
    </shiro:hasPermission> 
    <shiro:hasPermission name="DevPromptInfo_Audit">
