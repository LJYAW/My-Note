...
<html lang="en">
    ...
    <script type="text/javascript">
		...
		$(function(){ 
		   ...
		   function pageInit(){
    	    ...
    	    colModel : [ 
    	        	 ...
    	        	 {label:'PID',name : 'pid',index : 'pid',width : 50},
    	        	 {label:'PPID',name : 'ppid',index : 'ppid',width : 50},
    	        	 {label:'进程类型',name : 'procType',index : 'procType',width : 70},
    	        	 {label:'进程名称',name : 'procName',index : 'procName',width : 150},
    	        	 {label:'启动命令',name : 'startCmd',index : 'startCmd',width : 500},
    	        	 {label:'运行状态',name : 'runStatus',index : 'runStatus',width : 50,
        ...
        */
        ...
        function onAdd()
        {
        	var url = '${ctx}/fas/res/host/resServiceProcess/add.do';
        	layer_show('添加服务进程', url, 800, 400);
        }
        ...
        function onEdit()
        {
            ...
            if (ids==""||ids==null) { 
            	 layer.msg('请选择要修改的服务进程!', {icon: 7, time: 1500});
            }else if(ids.length>1){
              layer.msg('只能选择一条服务进程数据修改!', {icon: 7, time: 1500});
            }else{
              editRow(ids);
            }
        ...
        function editRow(id)
        {
        	var url = '${ctx}/fas/res/host/resServiceProcess/edit.do?id='+id;
        	layer_show('修改服务进程',url,800, 400);
        }
        ...
        { 
           var ids = jQuery("#list").jqGrid('getGridParam', 'selarrrow');
            if (ids==""||ids==null) { 
            	 layer.msg('请选择要查看的服务进程!', {icon: 7, time: 1500});
            }else if(ids.length>1){
              	layer.msg('只能选择一条服务进程数据查看!', {icon: 7, time: 1500});
            }else{
              	viewRow(ids);
            }
        }
        ...
        function viewRow(id)
        {
        	var url = '${ctx}/fas/res/host/resServiceProcess/view.do?id='+id;
        	layer_show('服务进程详情', url, 900, 400);
        }
        ...
        function onDelete()
        {
            var ids = jQuery("#list").jqGrid('getGridParam', 'selarrrow');
        	 if( ids.length == 0 ){
        		layer.msg('请先选择一条服务进程数据！', {icon: 7, time: 1500});
        	} else{
            	deleteRow(ids);
        	} 
        }
        ...
        function delRow(ids)
        {
        	var url = '${ctx}/fas/res/host/resServiceProcess/delete.do?ids='+ids;
        	$.ajax({
        		url: url,
        		dataType: 'json',
        ...
        function onExam(){
			var ids = jQuery("#list").jqGrid('getGridParam', 'selarrrow');
            if (ids==""||ids==null) { 
            	 layer.msg('请选择要审核的服务进程!', {icon: 7, time: 1500});
            }else if(ids.length>1){
              	layer.msg('只能选择一条服务进程数据进行审核!', {icon: 7, time: 1500});
            }else{
            	examRow(ids);
            }
		}
		...
		function examRow(id){
			var url = '${ctx}/fas/res/host/resServiceProcess/examAppService.do?id='+id;
        	$.ajax({
        		url: url,
        		dataType: 'json',
        		success: function(data) {
		...
		function onCheckProc(){
			var ids = jQuery("#list").jqGrid('getGridParam', 'selarrrow');
            if (ids==""||ids==null) { 
            	 layer.msg('请选择要检测的服务进程!', {icon: 7, time: 1500});
            }else if(ids.length>1){
              	layer.msg('只能选择一条服务进程数据进行检测!', {icon: 7, time: 1500});
            }else{
            	checkProcRow(ids);
            }
		}
		
		function checkProcRow(id){
			$("#trTxt").removeClass("dN");
			var url = '${ctx}/fas/res/host/resServiceProcess/checkProcStatus.do?id='+id;
        	$.ajax({
        		url: url,
        		dataType: 'json',
        		success: function(data) {
        			...
        			if( data.success!=null ){
        				layer.msg(data.success,{icon:1, time: 2500},function(){
        					onRefresh(); // 页面刷新
        	            });
        			}else if(data.error!=null){
        				layer.msg(data.error,{icon:7, time: 2500},function(){
        					onRefresh(); // 页面刷新
        	            });
        			}
        		}
...
<body>  
   <div id="btn" style="margin-top:2px;">
							...
							<button class="btn btn-white btn-success btn-bold"onclick="onCheckProc()" >
...
<div id="trTxt" class="dN" style="background-color:#FFDEAD; border:0px;top:300px; left:500px; position:absolute; z-index:4;height:31px;line-heigth:31px;text-align:left">
			<div  align="right">
			<span id="searching">
           			<img src="${ctx}/img/scaning.gif" alt="#" />&nbsp;&nbsp;进程检测中，请稍候...
           		</span>
			</div>
