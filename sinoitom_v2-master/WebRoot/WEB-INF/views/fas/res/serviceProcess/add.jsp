...
<script type="text/javascript">
	...
	function checkIP(){
		if($("#hostIp").val()!="" && $("#hostIp").val()!=null){
		   var devIpAddr=$("#hostIp").val();
		   if (!ipok(devIpAddr)) {
			   flag = false;
	...
	function getHostIp(){
		async: false ,
        $.getJSON(
            "${ctx}/fas/res/host/resServiceProcess/getHostIp.do",
            function (result) {
                if (result != null) {
                    $("#hostIp").empty();
                    for (var i = 0; i < result.resHosts.length; i++) {
                       var resHost= result.resHosts[i];
                       if(resHost.ipAddress=='${resServiceProcess.hostIp}'){
                         $("#hostIp").append("<option  value=\"" + resHost.ipAddress + "\" selected>" + resHost.ipAddress + "</option>");
                       }else{
                         $("#hostIp").append("<option  value=\"" + resHost.ipAddress + "\">" + resHost.ipAddress + "</option>");
                       }
                    }
                }
                getprocName();
	...
	function getprocName(){
		var hostip = $("#hostIp").find("option:selected").text();
		var procName = "Proc_"+hostip;
...
<body style="overflow: hidden;">
	<form id="formSave" action="${ctx}/fas/res/host/resServiceProcess/save.do" method="post">
		...
		<table align="center" class="tb_edit">
			...
			<tr>
				...
				<td>
<%--					<input id="hostIp" name="hostIp" value="${resServiceProcess.hostIp}" type="text" onchange="checkIP();" class="validate[required[服务器IP不能为空]] ip1"/>--%>
					<select id="hostIp" name="hostIp" onchange="checkHostIp();" class="ip1">
						<option></option>
					</select>
				</td>
				<td></td>
			...
			<tr>
				...
				<td colspan=5><input id="startCmd" name="startCmd" value="${resServiceProcess.startCmd }" type="text" ltype="text" class="validate[required[启动命令不能为空]] ip2"  /></td>
            </tr>
			<tr>
