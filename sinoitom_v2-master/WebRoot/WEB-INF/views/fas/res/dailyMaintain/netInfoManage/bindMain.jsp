...
<script src="${ctx}/static/ligerUI/js/plugins/ligerDialog.js" type="text/javascript"></script>
    <script type="text/javascript">
    	...
    	mainUrl='${ctx}/fas/res/dailyMaintain/netInfoManage/advancedBindSearch.do';
        $(function ()
        {
            ...
            gridMain = $("#maingrid").ligerGrid({
                columns: 
                    ...
                    { display:'Vlan', name:'vlanNo', width:70, align:'right'},
                     { display:'绑定状态', name:'bindStatus', width:80, align:'center', 
                    	render: function (row)
	                        {
	                        	var html='';
	                        	if(row.bindStatus==1){
	                        		html = '<span style=\"align:certer;line-height:22px;color:green;\">已绑定</span>';
	                        	}else{
	                        		html = '<span style=\"align:certer;line-height:22px;color:red;\">未绑定</span>';
	                        	}
	                            return html;
	                        }
	                },
                    { 
                    	display:'操作', width:100,name:'isoStatus',
                        render: function (row)
                        {
                        	...
                        	if(row.bindStatus==1){
	                            html += '| <span class="linkNormal" onmouseover="linkMouseOver(this);" onmouseout="linkMouseOut(this);" onclick="unbind(\'' + row.id + '\');">释放</span>';
                            }else{
                            	html += '| <span style="color:#808080" >释放</span>';
                            }
                            return html;
                        }
                    }
               		],  
				url: timeURL(baseUrl+'/searchBindAjaxPage.do'),	
        ...
        */
        ...
        {
        	var url = timeURL('${ctx}/fas/res/dailyMaintain/netInfoManage/advancedSearch.do');
     		window.parent.showDlg('高级查询', 800, 480, url);
     		//window.open(url,'', 'height=400, width=800, top=160, left=200, toolbar=no, menubar=no, scrollbars=no, resizable=yes,location=no, status=no');     
        } */
	...
	function multiView(switchId, ifIndex)
        {
        	var url = timeURL('${ctx}/fas/res/net/switch/interface/multiView.do?switchId='+switchId+'&ifIndex='+ifIndex);
     		window.parent.parent.showDlg('接口信息', 800, 480, url);     
        }
        
        
         function manage(id,switchId,indexId)
        {
        	var url = timeURL('${ctx}/fas/res/dailyMaintain/netInfoManage/manageBind.do?ipHostId='+id+'&switchId='+switchId+'&indexId='+indexId);
        ...
        function unbind(id){
            	...
            	var url = timeURL('${ctx}/fas/res/dailyMaintain/netInfoManage/unbinds.do?ids='+id);
            	$.ajax({
            		url: url,
            		dataType: 'json',
            		success: function(data) {
            			loading.close();
            			if( data.result == 'success' ){
                			window.parent.$.ligerDialog.success('解绑成功！');
                			onRefresh();         				
            			}
            			else{
                    		window.parent.$.ligerDialog.error(data.message);
...
<body style="padding:0px; overflow:hidden; "> 
    ...
    <div id="advancedSearchDlg" style="padding:0px;">
    	<form id="formAdvancedSearch" name="formAdvancedSearch" action="${ctx}/fas/res/dailyMaintain/netInfoManage/advancedBindSearch.do" method="post">
		<table align="center" class="tb_edit">
			...
			<tr>
				...
				<td>
					<select id="operStatus" name="filter_operStatus" style="width:146px;">
						<option value=""></option>
				...
				<td><input type="text" id="infoPointNo" name="filter_infoPointNo" /></td>
				<td></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><input type="text" id="serialNO" name="filter_serialNO" /></td>
				<td></td>
				<td align="right">Vlan：</td>
				<td><input type="text" id="vlanId" name="filter_vlanId" /></td>
				<td></td>
			</tr>
			<tr>
				<td align="right">使用人：</td>
				<td><input type="text" id="userId" name="filter_userId" /></td>
				<td></td>
				<td align="right">设备类型：</td>
				<td>
					<s:resSelect name="filter_ipHostTypeId" code="IpHostType" prompt="" ltype="select"/>
				</td>
			</tr>
			<tr>
				<td align="right">绑定状态：</td>
				<td>
					<select id="bindStatus" name="filter_bindStatus" style="width:146px;">
						<option value=""></option>
