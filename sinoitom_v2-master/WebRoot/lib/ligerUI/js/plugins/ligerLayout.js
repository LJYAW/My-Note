...
(function ($)
{
    ...
    $.ligerui.controls.Layout.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            if ($("> div[position=center]", g.layout).length > 0)
            {
                ...
                if ($("> div[position=centerbottom]", g.layout).length > 0)
                {
                    ...
                    g.centerBottom.content.append('<div class="l-layout-header-up"></div>'+'<div class="l-layout-header-down"></div>')
                    //set title
           ...
           $(".l-layout-header-up").click(function(event){
        	   if(parseInt(g.center.height())<=parseInt(g.centerBottom.height())){        		   
                   g.centerBottom.height(g.mask.height()-26-2);
                   g.center.height(26); 
                   g.centerBottom.content.height(g.centerBottom.height());
                   //使用上下箭头时修改centerBottom中的内容高度
                   var tabH=g.centerBottom.content.height();
         		   var tabLH=$(".l-tab-links").css("height");
         		   var tabHL=parseInt(tabH)-parseInt(tabLH);         		   
                   $(".l-tab-content").css({height:tabHL});
                   $(".l-tab-content-item").css({height:tabHL});
                   g.centerBottom.css({top:g.center.height()+3});
                   g.center.content.height(g.center.height());
                   g.centerBottomDropHandle.css({top:g.center.height()});
                // g.centerBottom.content.height(g.centerBottom.height()-g.centerBottom.header.height());
    
               }else if(parseInt(g.centerBottom.height())==26||g.center.height()>parseInt(g.centerB.height())){
                   g.center.height((g.mask.height()-2)/2-parseInt(g.centerBottomDropHandle.css('height'))/2);
            	   g.centerBottomDropHandle.css({top:g.center.height()});
                   g.centerBottom.height((g.mask.height()-2)/2-parseInt(g.centerBottomDropHandle.css('height'))/2);
                   g.centerBottom.content.height(g.centerBottom.height());
                   //修改centerBottom中的内容高度
                   var tabH=$(".l-tab").css("height");
         		   var tabLH=$(".l-tab-links").css("height");
         		   var tabHL=parseInt(tabH)-parseInt(tabLH);
                   $(".l-tab-content").css({height:tabHL});
                   $(".l-tab-content-item").css({height:tabHL});
                   g.centerBottom.css({top:g.center.height()+parseInt(g.centerBottomDropHandle.css('height'))});
                   g.center.content.height(g.center.height());
                  // g.centerBottom.content.height(g.centerBottom.height()-g.centerBottom.header.height());
               }
           });
           
           $('.l-layout-header-down').click(function(event) {
               if(parseInt(g.center.height())>=parseInt(g.centerBottom.height())){
                   g.centerBottom.height(26);
                   g.center.height(g.mask.height()-26-5);
                   g.centerBottom.content.height(g.centerBottom.height());
                 ...
                 //修改centerBottom中的内容高度
                   ...
                   g.center.content.height(g.center.height());
                   g.centerBottomDropHandle.css({top:g.center.height()});
                   g.centerBottom.css({top:g.center.height()+5});                   
                   //g.centerBottom.content.height(g.centerBottom.height()-g.centerBottom.header.height());
              }else if(parseInt(g.center.height())==26||parseInt(g.center.height())<parseInt(g.centerBottom.height())){
                   g.center.height((g.mask.height()-2)/2-parseInt(g.centerBottomDropHandle.css('height'))/2);
                   g.centerBottomDropHandle.css({top:g.center.height()});
                   g.centerBottom.height((g.mask.height()-2)/2-parseInt(g.centerBottomDropHandle.css('height'))/2);
               ...
               }else if(parseInt(g.centerBottom.height())==26){
                   g.center.height((g.mask.height()-2)/2-parseInt(g.centerBottomDropHandle.css('height'))/2);
                   g.centerBottomDropHandle.css({top:g.center.height()});
                   g.centerBottom.height((g.mask.height()-2)/2-parseInt(g.centerBottomDropHandle.css('height'))/2);
                   g.centerBottom.content.height(g.centerBottom.height());
        ...
        { 
            ...
            g.centerMHeight = g.center.height();
            ...
            if (g.center)
            {
                ...
                g.centerHeight = g.center.height();
                if (g.left)
                {
                    if (g.isLeftCollapse)
                    {
                ...
                g.center.width(g.centerWidth);
                g.center.height(g.middleHeight/2-parseInt(g.centerBottomDropHandle.css('height'))/2);
                g.centerBottom.height(g.middleHeight/2-parseInt(g.centerBottomDropHandle.css('height'))/2);
                g.center.content.height( g.center.height());

               /*g.centerWidth >= 0 && g.center.width(g.centerWidth);
                g.middleHeight >= 0 && g.center.height(g.middleHeight/2-parseInt(g.centerBottomDropHandle.css('height'))/2);
               g.middleHeight >= 0 && g.centerBottom.height(g.middleHeight/2-parseInt(g.centerBottomDropHandle.css('height'))/2);
                var contentHeight =g.centerHeight;
        ...
        _updateCenterBottom: function (isHeightResize)
        {
            ...
            var contentBHeight=g.centerBottom.height();
            if (g.centerBottom.header) contentHeight -= g.centerBottom.header.height();
            contentBHeight >= 0 && g.centerBottom.content.height(contentBHeight);
            ...
            if (g.centerBottom)
            {
                ...
                if (isHeightResize)
                {
                    ...
                    g.centerBottom.css({ top: centerTop + centerHeight+5});
            ...
            else if (g.yresize && g.yresize.diff != undefined)
            {
                ...
                else if (g.dragtype == 'centerbottomresize')
                {
                  ...
                  //修改centerBottom的内容高度使用cursor时
                    ...
                    g.center.content.height(g.center.content.height() + g.yresize.diff);
