		...
		//计算箭头平移位置
	        ...
	        this.computeArrowTranslate = function (dx,dy,radian,to){
	            ...
	            if( dx == 0 ){
	                ...
	                y = dy > 0 ? y-radiusY+arrawHalfW*3 : y+radiusY-arrawHalfW*3 ;
	                // y = dy > 0 ? y-radiusY: y+radiusY;


	            }else if( dy == 0 ){
	                x = dx > 0 ? x-radiusX-arrawHalfW : x+radiusX+arrawHalfW ;
	                // x = dx > 0 ? x+radiusX: x-radiusX ;

	            }else if(Math.abs(angle)<30){
	                ox = Math.ceil( (radiusX+arrawHalfW)*Math.cos( Math.abs(radian) ) );
	                oy = Math.ceil( (radiusY+arrawHalfW)*Math.sin( Math.abs(radian) ) );
...
//	   	    alert('mousedown=='+graph.graphModel);
            if(graph.graphModel=="Move"&&e.button==0){  // 平移模式时的拖动
    ...
    function EdgeBroken(name,from,to){
        ...
        this.drawArrow=function(from,to){
            ...
            var dx = to.x - from.x,
                ...
                graph.ctx.save();  
                graph.ctx.setTransform(1,0,0,1,0,0);  //恢复初始值            
                    if(this.x2>this.x4&&this.x1<this.x2){
...
Array.prototype.remove = function(val) {
   ...
   function saveLine(node){
	   ...
	   $.ajax({
		      url:proName+"/topo/topoLine/saveLine.do",
