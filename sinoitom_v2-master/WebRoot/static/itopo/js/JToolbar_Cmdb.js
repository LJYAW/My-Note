...
function showJTopoToobar(sta,name){
	...
	if(sta!=null){
		toobarDiv = $('<div class="jtopo_toolbar" style="width: auto; margin: 0px auto;">').html(''
			    ...
			    +'<div class="toolbar-btn toolbar-cp" title="缩小" id="zoomOut"><div class="toolbar-p toolbar-lessen"></div></div>'
			   
				+'<div class="toolbar-btn toolbar-c" title="平移" id="movemode" style="background-color:#fff;"><div class="toolbar-p toolbar-move"></div>	</div>'
				+'<div class="toolbar-btn toolbar-cp" title="框选" id="selectmode"><div class="toolbar-p toolbar-all"></div></div>'
				
				 +'<div class="toolbar-btn toolbar-c" title="显示关系" id="showEdgeText"><div class="toolbar-p toolbar-on-showText"></div></div>'
				 +'<div class="toolbar-btn toolbar-cp" title="隐藏关系" id="hideEdgeText"><div class="toolbar-p toolbar-on-hideText"></div></div>'
							
				
				+'<div class="toolbar-btn toolbar-c" title="直线" id="createLine" ><div class="toolbar-p toolbar-on-line"></div></div>'
...
//			    +'<div class="toolbar-btn toolbar-co" title="依赖" id="addBrokenEdgeArrow"><div class="toolbar-p toolbar-on-brokenEArrow"></div></div>'
			    ...
			    +'<div class="toolbar-btn toolbar-c" title="直线属性" id="line_name"><div class="toolbar-p toolbar-line_name"></div></div>'
			    +'<div class="toolbar-btn toolbar-cp" title="折线属性" id="brokenLine_name"><div class="toolbar-p toolbar-brokenline_name"></div></div>'
...
//			      +'<div class="toolbar-btn toolbar-co" title="依赖" id="addBrokenEdgeArrow"><div class="toolbar-p toolbar-on-brokenEArrow"></div></div>'
	...
	$('#selectmode').click(function(){
       ...
       $('#line_name').css("background-color","");
       $('#brokenLine_name').css("background-color","");
	});
...
//		graph.clear();
	...
	$('#line_name').click(function(){
		graph.lineName=true;
		graph.brokenLinename=false;
		changeGraphModel('Line_name');
		$('#selectmode').css("background-color","");
		$('#movemode').css("background-color","");
		$('#createLine').css("background-color","");
		$('#Lan').css("background-color","");
		...
		$('#addBrokenEdge').css("background-color","");
		$('#addBrokenEdgeArrow').css("background-color","");
		$('#line_name').css("background-color","#fff");
		$('#brokenLine_name').css("background-color","");
	});
//线属性
	$('#brokenLine_name').click(function(){
		graph.lineName=false;
		graph.brokenLinename=true;
		changeGraphModel('BrokenLine_name');
		$('#selectmode').css("background-color","");
		$('#movemode').css("background-color","");
		$('#createLine').css("background-color","");
		$('#Lan').css("background-color","");
		...
		$('#addBrokenEdgeArrow').css("background-color","");
		$('#line_name').css("background-color","");
		$('#brokenLine_name').css("background-color","#fff");
	});
	//显示线路信息
		    ...
		    graph.drawData()
		      for(var i=0;i<graph.EdgeText.length;i++){
		        for(var j=0;j<graph.childs.length;j++){
		          var BrokenEdges=graph.childs[j];
		          var edge=graph.childs[j];
		          if(edge.elementType=="Edge"&&edge.id==graph.EdgeText[i].textId){
		            graph.add(graph.EdgeText[i]);
		          }else if(BrokenEdges.elementType=="EdgeBroken"&&BrokenEdges.id==graph.EdgeText[i].textId){
		            graph.add(graph.EdgeText[i]);
		          }
		        }
		      }              
...
function createNode(node,x,y){
   var n=new Node(node.resName, x, y);
   n.id=node.id;
   n.objType=node.nodeType;
...
//   n.devStatus=node.devStatus;
   n.image=svgImgPath+node.nodeIcon;
   graph.add(n);
...
function loadNodeById(nodeId,resLabel){
	var textNode=getTextNodeByNode(nodeId);
	textNode.name = resLabel;
	graph.drawData();
}
...
function loadTopo(graphID){
    ...
    var lines,loadNodes,lineInfos,linesInfoArray,lans,lanNodes;
     var nodeArray=new Array();
         $.ajax({
    		type : 'post',
    		url: proName+'/cmdb/graph/getData.do?graphID='+graphID,
    		dataType: 'json',
    		cache : false, 
    		async: false,
    		success: function(data) {
    	...
    	for(var i=0;i<loadNodes.length;i++){
    	  var n=loadNodes[i];
    	   var node= createNode(n,n.posX,n.posY);//创建结点
		   nodeArray.push(node);
		   var resName = n.resName;
		   var resLabel = n.resLabel;
		   if(resLabel!=null && resLabel!=undefined && resLabel!=''){
			   createText(node, resLabel, n.namePosX,n.namePosY);//创建节点跟随文本节点
		   }else{
			   createText(node, resName, n.namePosX,n.namePosY);//创建节点跟随文本节点
		   }
...
//    	alert(JSON.stringify(lines));
    	for(var i=0;i<lines.length;i++){
		   ...
		   if(topoNodeA!=null&&topoNodeB!=null){
			   var edge=null;
			   var subType = link.relType;
			   if(subType!=undefined && subType>=0){
//				   alert("56789"+subType);
				   if(subType=="0"){
//					   折线箭头
					   edge=new EdgeBrokenArrow(link.relName,topoNodeA,topoNodeB,subType);
					   edge.isBroken=true;
					   edge.isDrawEdgeLabel=true;
				   }else if(subType=="1"){
					   edge=new EdgeSolidArrow(link.relName,topoNodeA,topoNodeB,subType);
					   
				   }else if(subType=="2"){
					   edge=new EdgeEmptyArrow(link.relName,topoNodeA,topoNodeB,subType);
				   }else if(subType=="3"){
					   edge=new DashEdgeSolidArrow(link.relName,topoNodeA,topoNodeB,subType);
				   }else if(subType=="4"){
//					   edge=new EdgeBroken(link.relName,topoNodeA,topoNodeB);
//					   edge.isBroken=true;
//					   edge.isDrawEdgeLabel=true;
					   edge=new Edge(link.relName,topoNodeA,topoNodeB);
				   }else{
					   edge=new Edge(link.relName,topoNodeA,topoNodeB);
				   }
			   }else{
...
//			   alert(link.id);
		      ...
		      graph.add(edge);//添加连线
		      //创建连线关系文本
		      var text = new TextNode(edge.name, edge.labelX, edge.labelY);
		  	  text.isDrawEdgeLabel=true;
		  	  text.textId=edge.id;
		  	  graph.EdgeText.push(text)
		      edgeArray.push(edge);
		   }
    	}
...
function getAutoData(){
	$.ajax({
	    		type : 'post',
	    		url: proName+'/cmdb/graph/autoLoad.do?graphId='+graphID,
	    		dataType: 'json',
	    		cache : false, 
	    		async: false,
	    		success: function(data) {
...
function saveNode(nodeStr){
	 $.ajax({
	      url:proName+"/cmdb/graph/saveNode.do",
	      
	      type:"POST",
 		  data:{"nodeStr":nodeStr,"graphId":graphID},
		...
		window.parent.$.ligerDialog.confirm('是否确认删除该节点信息！', function (yes){
			if(yes) {
					if(currentNode.nodeInfo!=null){  //新拖拽节点，还未入库
					   ...
					   $.ajax({
					      url:proName+"/cmdb/graph/deleteNode.do?nodeId="+currentNode.id+'&graphId='+graphID,
					      dataType:'html',
					      success:function(data){
					         if(data=="success"){
//					             window.parent.$.ligerDialog.success("节点删除成功!");
	...
	if(text == '关联实体资源'){
		var url = timeURL(proName+'/cmdb/graph/toAssociateBizApp.do?nodeId='+currentNode.id+'&graphId='+graphID+'&nodeInfo='+currentNode.nodeInfo);
     	showDlg('关联实体资源', 950, 400, url);    
	}
	if(text == '影响范围'){
		var url = timeURL(proName+'/cmdb/graph/toSphereOfInfluence.do?nodeId='+currentNode.id+'&graphId='+graphID);
     	showDlg('影响范围', 550, 450, url);   
	}
	        ...
	        success:function(data){
	          if(data.devStr!=null&&data.devStr!=""){
	           var node=data.nodeStr;
		             if(node.devClassCode==13){  
	                if(node.devTypeCode==2||node.devTypeCode==5){  //交换机，路由交换
	                    var url = timeURL(proName+'/fas/res/net/switch/ifDetail.do?id='+node.devID);
	                	...
	                	var url = timeURL(proName+'/fas/res/net/device/interface/main.do?devId='+node.devID);
						window.open(url,'', 'height=400, width=960, top=160, left=200, toolbar=no, menubar=no, scrollbars=no, resizable=yes,location=no, status=no');   
	                } 
		           }
	          }else{
	  ...
	  if(currentNode.id!=null){   //数据存已经存在该条line
		//alert(currentNode.id+"+++");
		 var url = timeURL(proName+'/topo/topoLine/editLine.do?lineId='+currentNode.id+'&graphId='+graphID);
            ...
            var url = timeURL(proName+'/cmdb/graph/toEditImpactFactor.do?lineId='+currentNode.id+'&graphId='+graphID);
	  		window.parent.showDlg('修改影响系数', 500, 200, url);    
    }else if(text=='删除连线'){
    
      window.parent.$.ligerDialog.confirm('是否确认删除该连线！', function (yes){
    	  if(yes){
        	if(currentNode.id==undefined){
        		graph.removeElement(currentNode);//说明是画布上新添加的线，还没有存入数据库
        	}
            if(currentNode.id!=null){
		         $.ajax({
		           url:proName+"/cmdb/graph/deleteLine.do?lineId="+currentNode.id+'&graphId='+graphID+'&type='+currentNode.elementType,
		           dataType:'html',
		           success:function(data){
		              if(data=="success"){
	 						 graph.removeElement(currentNode);
...
//	 						graph.EdgeText
	 						for(var i =0;i<graph.EdgeText.length;i++){
	 							if(graph.EdgeText[i].textId==currentNode.id){
	 								graph.removeElement(graph.EdgeText[i]);
	 								break;
	 							}
	 						}
...
function autoLoad(seedNodeId){
	   ...
	   for(var i=0;i<aloneNodeIds.length;i++){
	      ...
	      if(aloneNode){
	    	  var topoParentNode=createNode(aloneNode,rootNodeX+(i+1)*xgap,rootNodeY);
		 	  createText(topoParentNode, aloneNode.nodeName, rootNodeX+(i+1)*xgap, rootNodeY+30);
	      }
...
//				  var contexts=link.lineName.split("_");
	    	      var edge=new Edge(link.linkName,topoNodeA,topoNodeB);
			      edge.id=link.id;
			      graph.add(edge);
			      edgeArray.push(edge);
			   }
