/* 
* @Author: Marte
* @Date:   2016-11-30 14:44:04
* @Last Modified by:   Marte
* @Last Modified time: 2016-11-30 14:44:08
*/
...
function f_onAdd()
{
    ...
    if( isTreeList ){
        ...
        if(rows.length>0 && isOneTree) { 
            window.parent.parent.$.ligerDialog.error('请选择上级'+objName+'!'); 
            return; 
        }
        if(rows.length>1){ 
            window.parent.parent.$.ligerDialog.error('只能选择一条上级'+objName+'记录!'); 
            return; 
        }
        if( rows.length>0 ){
...
function getAllSelectId(action)
{
    var rows = gridMain.getSelectedRows();
    if(rows.length==0){ 
        window.parent.$.ligerDialog.warn('至少选择一条记录！'); 
        return ''; 
    }
    var id = rows[0].id;
...
function f_onDelete(message)
{
    ...
    if( id != null && id != '' ){
        window.parent.$.ligerDialog.confirm(message, function (yes){
            if(yes) {
                deleteRow(id);
            }
        });
...
function viewRow(id)
{
    ...
    window.parent.parent.showDlg('查看'+objName, editDlgWidth, editDlgHeight, url);     
}
...
function moveRow(id)
{
    ...
    window.parent.showDlg('移动'+objName, moveDlgWidth, moveDlgHeight, url);     
}
...
function f_onClear()
{
    ...
    window.parent.showDlg('清除'+objName, clearDlgWidth, clearDlgHeight, url);     
    return false;
}
...
function f_onImport()
{
    ...
    window.parent.showDlg('导入'+objName, impDlgWidth, impDlgHeight, timeURL(url));   
    return false;
}
...
function f_onExport()
{
    ...
    window.parent.showDlg('导出'+objName, expDlgWidth, expDlgHeight, timeURL(url));   
    return false;
}