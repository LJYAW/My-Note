...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        url: false,                             //ajax url
        usePager: true,                         //是否分页
        usePagerDef: false,                         //是否分页
        page: 1,                                //默认当前页 
        pageSize: 10,                           //每页默认的结果数
        pageSizeDef: 100,                           //每页默认的结果数
		...
		toolbarDef: null,                           //工具条,参数同 ligerToolbar的
        headerImg: null                        //表格头部图标
    };
    $.ligerDefaults.GridString = {
        errorMessage: '发生错误',
        pageStatMessage: '显示从{from}到{to}，内存记录数： {total} 条 。每页显示：{pagesize}',
		pageStatMessageDef: '总记录数： {total} 条 ',
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
			...
			//自定义分页div
			gridhtmlarr.push("         <div class='l-panel-barDef'>");
			gridhtmlarr.push("           <div class='l-bar-group  l-bar-defpage'>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
            gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnfirstDef'><span></span></div>");
            gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnprevDef'><span></span></div>");
            gridhtmlarr.push("                </div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'><span class='pcontrolDef'> <input type='text' size='4' class='pageInput' value='1' style='width:20px' maxlength='3' /> / <span></span></span></div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
            gridhtmlarr.push("                     <div class='l-bar-button l-bar-btnnextDef'><span></span></div>");
            gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnlastDef'><span></span></div>");
            gridhtmlarr.push("                </div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
			gridhtmlarr.push("                <div class='l-bar-group  l-bar-messageDef'><span class='l-bar-textDef'></span></div>");
			gridhtmlarr.push("            </div>");
			gridhtmlarr.push("           </div>");
			...
			g.toolbarDef = $(".l-panel-barDef:first", g.grid);
        ...
        loadData: function (loadDataParm)
        {
            ...
            if (!p.newPage) p.newPage = 1;
			 if (!p.newPageDef) p.newPageDef = 1;
        ...
        */
        ...
        //自定义改变分页
        changePageDef: function(ctype)
        {
            var g = this, p = this.options;
            if (g.loading) return true;
            if (p.dataAction != "local" && g.isDataChanged && !confirm(p.isContinueByDataChanged))
                return false;
			p.pageCountDef = parseInt($(".pcontrolDef span", g.toolbar).html());
            switch (ctype)
            {
            		case 'firstDef': if (p.pageDef == 1) return; p.newPageDef = 1; break;
					case 'prevDef': if (p.pageDef == 1) return; if (p.pageDef > 1) p.newPageDef = parseInt(p.pageDef) - 1; break;
					case 'nextDef': if (p.pageDef >= p.pageCountDef) return; p.newPageDef = parseInt(p.pageDef) + 1; break;
					case 'lastDef': if (p.pageDef >= p.pageCountDef) return; p.newPageDef = p.pageCountDef; break;
					case 'inputDef':
                    var nv = parseInt($('.pcontrolDef input', g.toolbar).val());
                    if (isNaN(nv)) nv = 1;
                    if (nv < 1) nv = 1;
                    else if (nv > p.pageCountDef) nv = p.pageCountDef;
                    $('.pcontrolDef input', g.toolbar).val(nv);
                    p.newPageDef = nv;
                    break;
            }
						if (p.newPageDef == p.pageDef) return false;
            if (p.newPageDef == 1)
            {
				$(".l-bar-btnprevDef span", g.toolbar).addClass("l-disabled");
				$(".l-bar-btnfirstDef span", g.toolbar).addClass("l-disabled");
            }
            else
            {
				$(".l-bar-btnprevDef span", g.toolbar).removeClass("l-disabled");
				$(".l-bar-btnfirstDef span", g.toolbar).removeClass("l-disabled");
            }
           
			if (p.newPageDef == p.pageCountDef)
            {
                $(".l-bar-btnnextDef span", g.toolbar).addClass("l-disabled");
                $(".l-bar-btnlastDef span", g.toolbar).addClass("l-disabled");
            }
            else
            {
                $(".l-bar-btnnextDef span", g.toolbar).removeClass("l-disabled");
                $(".l-bar-btnlastDef span", g.toolbar).removeClass("l-disabled");
            }

            g.trigger('changePageDef', [p.newPageDef]);
            if (p.dataAction == "server")
            {
                g.loadData(p.where);
            }
        ...
        _showData: function ()
        {
            ...
            if (p.usePager)
            {
                ...
                //更新自定义分页
                p.pageDef = p.newPageDef;
                p.totalDef = g.data[p.record];
                p.pageCountDef = Math.ceil(p.totalDef / p.pageSizeDef);
                if (!p.pageCountDef) p.pageCountDef = 1;
					g._buildPagerDef();
            }
            //加载中
		...
		//自定义分页
		 _buildPagerDef: function ()
        {
            var g = this, p = this.options;
            $('.pcontrolDef input', g.toolbar).val(p.pageDef);
            if (!p.pageCountDef) p.pageCountDef = 1;
            $('.pcontrolDef span', g.toolbar).html(p.pageCountDef);
            var r1 = parseInt((p.pageDef - 1) * p.pageSizeDef) + 1.0;
            var r2 = parseInt(r1) + parseInt(p.pageSizeDef) - 1;
            if (!p.totalDef) p.totalDef = 0;
            if (p.totalDef < r2) r2 = p.totalDef;
            if (!p.totalDef) r1 = r2 = 0;
            if (r1 < 0) r1 = 0;
            if (r2 < 0) r2 = 0;
            var stat = p.pageStatMessageDef;
            stat = stat.replace(/{total}/, p.totalDef);
            $('.l-bar-textDef', g.toolbar).html(stat);
            if (!p.totalDef)
            {
                $(".l-bar-btnprevDef span,.l-bar-btnnextDef span,.l-bar-btnfirstDef span,.l-bar-btnlastDef span", g.toolbar)
                    .addClass("l-disabled");
            }
            if (p.pageDef == 1)
            {
            ...
            else if (p.pageDef > p.pageCountDef && p.pageCountDef > 0)
            {
                $(".l-bar-btnprevDef span", g.toolbar).removeClass("l-disabled");
                $(".l-bar-btnfirstDef span", g.toolbar).removeClass("l-disabled");
            }
            if (p.pageDef == p.pageCountDef)
            {
                $(".l-bar-btnnextDef span", g.toolbar).addClass("l-disabled");
                $(".l-bar-btnlastDef span", g.toolbar).addClass("l-disabled");
            }
		...
		//获取自定义分页信息
		getDefPageSize: function ()
        {
            var g = this, p = this.options;
			return p.pageSizeDef;
		},
        ...
        _getSrcElementByEvent: function (e)
        {
			...
			if(r.toolbarDef){
				r.firstDef = fn("l-bar-btnfirstDef");
                r.lastDef = fn("l-bar-btnlastDef");
				r.nextDef = fn("l-bar-btnnextDef");
                r.prevDef = fn("l-bar-btnprevDef");
			}
        ...
        _onClick: function (e)
        {
            ...
            else if (src.toolbar)
            {
				...
				if(src.toolbarDef){
					if (src.firstDef)
	                {
	                    if (g.trigger('toFirstDef', [g.element]) == false) return false;
	                    g.changePageDef('firstDef');
	                }
					else if (src.prevDef)
					{
						if (g.trigger('toPrevDef', [g.element]) == false) return false;
						g.changePageDef('prevDef');
					}
					else if (src.nextDef)
					{
						if (g.trigger('toNextDef', [g.element]) == false) return false;
						g.changePageDef('nextDef');
					}
					else if (src.lastDef)
	                {
	                    if (g.trigger('toLastDef', [g.element]) == false) return false;
	                    g.changePageDef('lastDef');
	                }
