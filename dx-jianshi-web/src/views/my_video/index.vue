<template>
  <div class="container">
    ...
    <div class="d-flex mt-26px align-items-center justify-content-between">
      <div v-for="(item, index) in menuList"
        :key="index"
        :class="['fc-999 py-5px mr-20px', { active: activeIndex == index }]"
      ...
      <div class="">
        <el-input style="width:350px"
          ...
          suffix-icon="iconfont iconsearch fz-16px"
          v-model="keyword">
        </el-input>
      </div>
    </div>
    ...
    <div v-else>
      <div class="text-center mt-100px"
        v-if="list.length < 1">
        <img src="@/assets/images/no_videos.png"
          height="240"
          width="240">
          ...
          <el-row :gutter="20">
            <el-col :span="5"
              ...
              <div class="item">
                ...
                <div class="img-wrap"
                  @click="videoMshow(item)"
                  style="height:137px">

                  <div v-if="item.status =='生成中' || item.status == '预处理中'"
                    class="is_ing h-100 border-none">
                    <img class="ob-cover h-100"
                      src="@/assets/images/created_ing.png">
                  </div>
                  <div v-else
                    class="h-100">
                    <i class="iconfont icontupian-tianchong fz-30px"
                      v-if="!item.cover_pic && item.status !=='生成中'"></i>
                ...
                <div class="w-100 fc-999 text-wrap"
                  style="background:white;padding:16px">
                  <div style="color:#202020;line-height:20px;min-height: 40px"
                  ...
                  <div class="mt-18px">
                    ...
                    <p class="fz-12px mt-7px w-100 flex-start text-ellipsis"
                      v-if="item.status !== '生成中'">
                      <span> 生成时间：</span>
                      <span style="transform: scale(1); word-break: break-all;">{{ item.finished_time }}</span>
                    </p>
                    <p v-else-if="item.finished_time_by_now"
                      class="fz-12px mt-7px text-ellipsis">
                      预计生成用时：约5分钟
                    </p>
                  </div>
                ...
                <div class="icon-wrap d-flex justify-content-between w-100">
                  ...
                  <a @click.stop="resetEdit(item)"
                    class=" ml-10px">
                    <i class="iconfont iconbianji fc-white"></i>
                  </a>

                  <a @click.stop="deleteVideo(item)"
