...
export default {
  ...
  mounted: function() {
    ...
    if (this.dragHandle) {
      const dragHandles = Array.prototype.slice.call(
    ...
    if (this.dragCancel) {
      const cancelHandles = Array.prototype.slice.call(
  ...
  methods: {
    deselect() {
      // 说明还是在元素内部
      const HEADER_H = 60 // 这个高度 是剪辑页面的 header 高度
      var mouseX = this.getMouseXY().x
      var mouseY = this.getMouseXY().y - HEADER_H
      if (
        mouseX > this.rect.left &&
        mouseX < this.rect.left + this.rect.width &&
        mouseY > this.rect.top &&
        mouseY < this.rect.top + this.rect.height
      ) {
        console.log(' 说明还是在元素内部')
        return
      }
    ...
    up(ev) {
      ...
      this.$emit('changeover', this.rect)
    },
    getMouseXY(e) {
      // 函数用于获取鼠标的位置
      var x = 0
