 ...
 * @LastEditTime: 2020-12-22 20:01:28
...
<template>
  <iframe
    ...
    height="100%"
    @load="loaded"
    id="videoEditIfame"></iframe>

</template>

<script>
import { getVideoDuantion } from '@/utils/check_video_size.js'
...
export default {
  ...
  methods: {
    async loaded() {
      ...
      let track_H = this.iframe.contentWindow.document.getElementsByClassName('trackHeader')[0]
      let ul = track_H.getElementsByTagName('ul')[0]
      let liExport = ul.getElementsByClassName('export-btn')[0]
    ...
    appendFlag() {
      ...
      let liFlag = ul.getElementsByTagName('li')[0]
      let liExport = ul.getElementsByClassName('export-btn')[0]
      liExport.style.display = 'block'

      let exportBtn = liExport.getElementsByTagName('div')[0]
    ...
    receiveMessage(event) {
      ...
      if (event.data.cmd == 'exportAllData') {
        ...
        let video_item = Object.assign(this.video_item, data.cutDetails, { decorateList: data.decorateList[0] }, { videoSize: data.videoSize })
        this.modalM.submit(video_item)
      }
    ...
    async playerLoadeddata() {
      ...
      if (this.video_item.time_start_str) {
        ...
        let scroll = this.iframe.contentWindow.document.getElementsByClassName('edit-pad')[0]
    ...
    onResize() {
      let { start_ms, end_ms } = event.data.data
      let duration = Math.floor((end_ms - start_ms) / 1000)
      let originDuration = Math.floor(this.select_item.duration / 1000)
