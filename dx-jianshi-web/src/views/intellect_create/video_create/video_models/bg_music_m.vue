    ...
    <model :id="'bgMusicM'" ref="bgMusicM" @close="close">
      <div slot="title">
        <!-- <div class="handel-title">
          <span :class="{'is-active': showType == '简视推荐'}" @click="showType = '简视推荐'">简视推荐</span>
          <span :class="{'is-active': showType == '本地音乐'}" @click="showType = '本地音乐'">本地音乐</span>
        </div> -->
        <hander-title @showTypeHandler="showTypeHandler" :typeArr="['简视推荐', '本地音乐']"></hander-title>

        <audioPlay :url="audio_url" :play="play" />
      </div>
      <div slot="body">
        <div class="content-body">
          <div v-if="showType == '简视推荐'">
            ...
            <div class="search-key mb-20px">
              <el-input v-model="key_word" placeholder="输入关键词以搜索音乐名称" @input="searchData"></el-input>
              <vsvg icon="iconsousuoicon-01" @click.native="searchData"></vsvg>
            </div>
            <div>
              <ul class="muisc-list d-flex flex-column">
                <li
                  v-for="(item, index) in bg_muisc_list"
                  :key="index"
                  @click="changeId(item, index)"
                  class="item flex-1 d-flex justify-content-between align-items-center px-15px"
                >
                  <div>
          ...
          <div v-if="showType == '本地音乐'" class="my-muisc-list">
...
import * as modelSet from '../../js/model_set'
...
export default {
  ...
  components: { audioPlay, handerTitle },
  ...
  methods: {
    ...
    getFileUrl(url, file) {
      ...
      console.log('getFileUrl -> file', file)
    ...
    searchData() {
      let list = this.copyList(this.bg_music)
      delay(() => {
        this.bg_muisc_list = list.filter(item => {
          return item.name.indexOf(this.key_word) >= 0
        })
      }, 500)
    },
    changeType(name) {
