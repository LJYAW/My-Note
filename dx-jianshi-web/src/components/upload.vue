<template>
  <div class="d-flex align-items-center upload">
    ...
    <a class="fc-bb d-flex flex-column align-items-center" @click="updataImg()">
      <!-- <i class="iconfont iconsolid_add fz-70px"></i> -->
      <div class="btn-style">{{ button_name }}</div>
      <slot></slot>
    </a>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    limitFileSize(size) {
      ...
      if (this.file_type.includes('audio')) {
        MAX_SIZE = 20 * 1024
      }
      if (size > MAX_SIZE) {
