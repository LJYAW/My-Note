...
export default {
  ...
  mounted: function() {
    ...
    // 父组件宽度改变的 函数 时间线 这个功能暂时不写
    // this.$bus.$on('checkParentSize', () => {
    //   this.initBaseData()
    // })

    this.$emit('onCreated', this.left, this.top, this.width, this.height)
  },
  ...
  methods: {
    ...
    getParentSize() {
      if (this.parent) {
        ...
        let parentW = parseInt(style.getPropertyValue('width'), 10)
        const parentH = parseInt(style.getPropertyValue('height'), 10)
        if (this.parentW > 1) parentW = this.parentW
    ...
    deselect(e) {
      try {
        const target = e.target || e.srcElement
        const classNameHandle = target.className.indexOf('handle') > -1
        const regex = new RegExp(this.className + '-([trmbl]{2})', '')

        if (!this.$el.contains(target) && !regex.test(target.className) && !classNameHandle) {
