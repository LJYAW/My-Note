 ...
 * @LastEditTime: 2021-10-22 14:30:04
...
export default {
  ...
  methods: {
    ...
    async getFileUrl(url, file) {
      ...
      const imgData = await sdkUploadFile(imgFile, 'mask/')

      const videoData = await sdkUploadFile(file, 'mask/', (total) => {
        const progress = Math.min((Math.floor(total / file.size * 100)), 100)
        this.progress = progress
      })

      const params = {
        video_local_key: videoData.body.key,
        video_url: videoData.body.location,
        cover_url: imgData.body.location,
        titles: this.title,
