 ...
 * @LastEditTime: 2021-07-15 18:04:14
...
<template>
  <div class="add-user">
    ...
    <el-form ref="form" :model="form" :inline="true" class="form" :rules="rules">
      <el-form-item class="item-list" prop="company">
        <span class="form-label">公司名称</span>
        <el-input v-model="form.company" placeholder="请输入内容" />
      </el-form-item>
      ...
      <el-form-item class="item-list" prop="names">
        <span class="form-label">账号信息</span>
        <el-input id="valueInput" v-model="form.names" v-Alphabet placeholder="请输入内容" />
      </el-form-item>
      ...
      <el-form-item class="btn">
        <el-button type="primary" class="add" @click="add('form')">保存</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        company: '',
        create_time: parseInt(new Date().getTime() / 1000) || null,
        names: '',
        endtime: '',
        passwd: '',
        saleuser: '',
  ...
  computed: {
    setDateTime() {
      const time = getYMDHMS(new Date().getTime())
      return time
    }
  },
  watch: {
  ...
  methods: {
    ...
    async submitData(params) {
      const res = await this.$post('/v1/user/', params)
      if (res.err) {
