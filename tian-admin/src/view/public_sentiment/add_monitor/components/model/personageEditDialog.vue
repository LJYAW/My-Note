 ...
 * @LastEditTime: 2021-04-29 12:15:53
...
<template>
  <div>
    ...
    <el-table :data="dialogTableDataPersonage" style="width: 100%; margin-bottom: 20px">
      <el-table-column prop="name" label="公司" />
      <el-table-column prop="personage" label="人物" />
...
export default {
  ...
  methods: {
    ...
    async initTopics() {
      const { res } = await this.$get('/api/topics', { limit: 10000, query: `types:2` })
    ...
    addItem() {
      this.$emit('addList', this.dialogSelectVal, 'personage')
    },
    deleteItem(scope) {
      this.$emit('deleteList', scope.$index, 'personage')
    },
    monitor(item) {
